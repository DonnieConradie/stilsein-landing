...
window.currentLang = localStorage.getItem('stilsein_lang') || 'af';

window.changeLanguage = function(lang) {
  // CRITICAL FIX 5: Validate supported language
  ...
  // CRITICAL FIX 1: Update HTML lang attribute dynamically for SEO & Accessibility
  document.documentElement.lang = lang;

  // Verwyder FOUC klaskode (wys skerm as dit weggesteek is weens laai-tyd)
  document.documentElement.classList.remove('lang-switching');

  // 1. Dateer innerHTML op
  document.querySelectorAll('[data-i18n]').forEach(el => {
    const key = el.getAttribute('data-i18n');
    if (translations[lang][key]) {
      el.innerHTML = translations[lang][key];
    } else {
      // CRITICAL FIX 4: Add missing translation fallback warning
      console.warn(`[StilSein i18n] Missing translation for key: ${key} (${lang})`);
    }
  });

  // 2. Dateer form-placeholders op
  document.querySelectorAll('[data-i18n-placeholder]').forEach(el => {
    const key = el.getAttribute('data-i18n-placeholder');
    if (translations[lang][key]) el.setAttribute('placeholder', translations[lang][key]);
  });

  // CRITICAL FIX 6: Dateer meta/ARIA attribute op (aria-label, title, alt)
  ...
  // 4. Dateer visuele taal-knoppies op
  document.querySelectorAll('.lang-btn').forEach(btn => {
    btn.classList.remove('bg-jakkals-orange', 'text-stilsein-dark');
    btn.classList.add('text-stilsein-muted', 'hover:text-white', 'bg-transparent');
  });

  document.querySelectorAll(`.lang-btn-${lang}`).forEach(btn => {
    btn.classList.remove('text-stilsein-muted', 'hover:text-white', 'bg-transparent');
    btn.classList.add('bg-jakkals-orange', 'text-stilsein-dark');
  });

  // 5. Hardeware Toggle opdatering
